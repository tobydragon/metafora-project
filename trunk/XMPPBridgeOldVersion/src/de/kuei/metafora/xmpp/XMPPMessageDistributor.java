...
public class XMPPMessageDistributor implements Runnable {
	...
	private List<XMPPMessageListenerLanguage> languageListeners = null;
	private List<XMPPMessageTimeListenerLanguage> timeLanguageListeners = null;
	private List<XMPPMessageListener> listeners = null;
	private List<XMPPMessageTimeListener> timeListeners = null;
	private List<XMPPMessageTimeListenerLanguageSubject> timeLanguageSubjectListeners = null;

	private List<XMPPPresenceListener> presenceListeners = null;
	...
	public XMPPMessageDistributor(XMPPBridgeCurrent bridge) {
		...
		languageListeners = Collections
				.synchronizedList(new Vector<XMPPMessageListenerLanguage>());
		timeLanguageListeners = Collections
				.synchronizedList(new Vector<XMPPMessageTimeListenerLanguage>());
		listeners = Collections
				.synchronizedList(new Vector<XMPPMessageListener>());
		timeListeners = Collections
				.synchronizedList(new Vector<XMPPMessageTimeListener>());
		timeLanguageSubjectListeners = Collections
				.synchronizedList(new Vector<XMPPMessageTimeListenerLanguageSubject>());

		presenceListeners = Collections
				.synchronizedList(new Vector<XMPPPresenceListener>());
	}
	...
	public void addTimeLanguageSubjectListener(
			XMPPMessageTimeListenerLanguageSubject listener) {
		timeLanguageSubjectListeners.add(listener);
	}

	public void removeTimeLanguageSubjectListener(
			XMPPMessageTimeListenerLanguageSubject listener) {
	...
	public void run() {
		while (run) {
			if (languageListeners.size() > 0
					...
					|| timeListeners.size() > 0 || presenceListeners.size() > 0
	...
	public void processPacket(Packet packet) {
		if (packet instanceof Message) {
			...
			if (msg.getBody() == null) {
				System.err
						.println("XMPPMessageDistributor.processPacket: packet dropped because message was null!");
				return;
			}
