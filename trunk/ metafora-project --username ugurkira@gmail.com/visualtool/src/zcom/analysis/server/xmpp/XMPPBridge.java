...
public class XMPPBridge implements PacketListener, MessageListener,
	...
	public static XMPPBridge getConnection(String name) {
		...
		if (DEBUG)
			System.err
					.println("XMPPBridge: Connection name isn't known. Return null.");
		return null;
	}
	...
	public static XMPPBridge getTestConnection() {
		...
		try {
			createConnection("test", "testuser", "didPfT",
	...
	public static void createConnection(String connectionname, String user,
		...
		if (parameters.containsKey(connectionname)) {
			...
			if (param[0].equals(user) && param[1].equals(password)
				...
				//	logger.info("XMPPBridge: The connection " + connectionname
				//	+ " was already created.");
				if (DEBUG)
					System.err.println("XMPPBridge: The connection "
							+ connectionname + " was already created.");
			} else {
				//logger.error("XMPPBridge: The connection " + connectionname
	...
	private MultiUserChat multichat = null;
	...
	private XMPPBridge(String server, String user, String password,
		...
		//logger.debug("XMPPBridge: create new xmpp connection: " + this.server
		//	+ ", " + this.user + ", " + this.password + ", "
		//	+ this.chatroom + ", " + this.alias + ", " + this.device);
			...
			try {
				...
				if (register) {
					try {
						...
						//	logger.error("XMPPBrdige: connect: create new account: "
						//	+ e.getMessage());
						if (DEBUG)
							e.printStackTrace();
					}
		...
		if (connection.isConnected()) {
			if (multichat != null) {
				if (multichat.isJoined()) {
					if (!multichat.getRoom().toLowerCase()
							.equals(chat.toLowerCase())) {
			...
			try {
				//	logger.debug("XMPPBridge: connectToChat: Join chat as "
				//	+ this.alias);
				multichat.join(this.alias);
			} catch (XMPPException e) {
				//logger.debug("XMPPBridge: connectToChat: " + e.getMessage()
				//	+ " -> Retry with other user alias.");
				if (DEBUG)
					e.printStackTrace();
				...
				try {
					...
					//logger.debug("XMPPBridge: connectToChat: Try with "
					//	+ this.alias);
	...
	public void processMessage(Chat chat, Message message) {
		...
		for(int i=0; i<timeListeners.size(); i++){
			timeListeners.get(i).newMessage(name, text, name, new Date());
		}
	...
	public void chatCreated(Chat chat, boolean createdLocally) {
		if (!createdLocally) {
			if (DEBUG)
				System.err.println("new chat: " + chat);
