...
public class ExtendedPieChart extends  VerticalPanel {
	...
	private IndicatorEntity  groupingEntity;
	
	
	public ExtendedPieChart(IndicatorEntity  _entity, EntityViewModel _model, ClientInterfaceManager controller){
		this.groupingEntity=_entity;
		
		this.setId("pieChartVerticalPanel");
		model=_model;
		this.removeAll();
		this.add(createPieChart(model.getEntityDataTable(groupingEntity), controller));
	...
	public PieChart createPieChart(DataTable data, ClientInterfaceManager controller) {  
		    ...
		    pieChart.addSelectHandler(new PieChartSelectionHandler(pieChart, model, controller));
		    return pieChart;
	}
