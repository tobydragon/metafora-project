...
public class XMPPBridgeCurrent implements RosterListener{
	...
	private static String PASSWORD = "didPfT";
	private static String ALIAS = null;
	private static String DEVICE = "VipOWeb";
	...
	public XMPPBridgeCurrent() {
		...
		multiUserChats = Collections.synchronizedMap(new HashMap<String, MultiUserChat>());
	...
	public void login(boolean register){
				...
				try {
					...
					connection.connect();
						...
						try {
							...
							System.err.println("XMPPBridge.login: login after account creation failed!");
							ex.printStackTrace();
						}
					} catch (XMPPException ex) {
						System.err.println("XMPPBridge.login: connect after account creation failed!");
						ex.printStackTrace();
					}
				} catch (XMPPException e1) {
					System.err.println("XMPPBridge.login: create Account failed!");
					e1.printStackTrace();
				}
			}else{
		...
		if(connection.isConnected() && connection.isAuthenticated()){
			...
			if(distributor == null){
				distributor = new XMPPMessageDistributor();
			}
			...
			for(RosterEntry entry : roster.getEntries()){
				if(!userToName.containsKey(entry.getUser())){
					...
					System.out.println("Roster: contact: Name:"+entry.getName()+" User: "+entry.getUser());
				}else{
					System.out.println("Roster: known contact: Name:"+entry.getName()+" User: "+entry.getUser());
				}
			}
	...
	public String getPresenceString(String user){
		if(roster != null){
			...
			System.err.println("XMPPBridge.getPresenceString: roster not initialized!");
		}
	...
	public void watcher(){
		...
		if(!connection.isConnected() || !connection.isAuthenticated()){
			System.err.println("XMPPBridge.watcher: Bridge is not connected! Try to reconnect.");
			
			shutdown();
			login(false);
			
			if(connection.isConnected() && connection.isAuthenticated()){
				System.err.println("XMPPBridge.watcher: reconnect successful. Try to restart chats.");
				restartMultiChats();
			}else{
	...
	private void restartMultiChats(){
		for(String chat : multiUserChats.keySet()){
			System.err.println("XMPPBridge.restartMultiChats: restart chat "+chat);
	...
	public void connectToChat(String chat) {
		if(connection.isConnected() && connection.isAuthenticated() && distributor != null){
			...
			System.err.println("XMPPBridge.connectToChat: user not logged in or not authorized!");
		}
	}

	private void joinMultiUserChat(MultiUserChat muc){
		try {
			muc.join(alias);
		} catch (XMPPException e) {
			try {
				...
				System.err.println("XMPPBridge.joinMultiUserChat: multiuserchat could not be joined!");
				e1.printStackTrace();
			}
		}
	}
	
	public void sendMessageToMultiUserChat(String chat, String message, String subject) {
		if(connection.isConnected() && connection.isAuthenticated()){
			MultiUserChat muc = multiUserChats.get(chat);
			...
			if(muc != null){
				...
				try {
					...
					System.err.println("XMPPBridge.sendMessageToMultiUserChat: message send error!");
					e.printStackTrace();
				}
			}else{
				System.err.println("XMPPBridge.sendMessageToMultiUserChat: chat not found!");
			}
		}else{
			System.err.println("XMPPBridge.sendMessageToMultiUserChat: user not logged in or not authorized!");
		}
	}
	...
	public Vector<String> getChatParticipants(String chat){
		...
		if(muc != null){
			try {
				...
				System.err.println("XMPPBridge.getChatParticipants: XMPP exception!");
				e.printStackTrace();
			}
		}
