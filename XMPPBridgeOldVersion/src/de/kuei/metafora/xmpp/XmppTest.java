...
public class XmppTest {
	...
	private static void analysisRequest() {
		...
		bridge.getDistributor().addTimeLanguageSubjectListener(
				new XMPPMessageTimeListenerLanguageSubject() {
		...
		bridge.sendMessageToUser(request,
				"metaforaservice@metafora.ku-eichstaett.de", null);
	...
	private static void unityTest() {
		...
		bridge1.sendMessageToMultiUserChat(chat, "A message from connection 1",
				null, "en");
		bridge2.sendMessageToMultiUserChat(chat, "A message from connection 2",
				null, "en");
		bridge3.sendMessageToMultiUserChat(chat, "A message from connection 3",
				null, "en");
	...
	private static void compatible() {
		XMPPBridge.createConnection("logger", "KUU0004", "KUU0004",
				"logger@conference.metafora.ku-eichstaett.de", "KU User 0004",
				"newBridgeTest");
		...
		bridge.registerTimeListener(new XMPPMessageTimeListener() {
			...
			public void newMessage(String user, String message, String chat,
					Date time) {
				System.err.println("MessageTimeListener: " + user + ": "
						+ message);
			}
		});
	...
	private static void testMultiUser() {
		...
		try {
			...
			XMPPBridge.getConnection("planningcommand").registerTimeListener(
					new XMPPMessageTimeListener() {

						@Override
						public void newMessage(String user, String message,
								String chat, Date time) {
			...
			XMPPBridge.getConnection("planningtoolinput").registerTimeListener(
					new XMPPMessageTimeListener() {
			...
			XMPPBridge.getConnection("planningtoolinput").sendMessage(
					"eine nachricht");
	...
	private static void test() {
		...
		XMPPBridgeCurrent.setServer("metafora.ku-eichstaett.de");
		...
		try {
			...
			if (distributor != null) {
				distributor
						.addTimeLanguageSubjectListener(new XMPPMessageTimeListenerLanguageSubject() {
			...
			bridge.sendMessageToUser("a private message", "KUU0004",
					"a subject", "en");
			bridge.sendMessageToUser("eine private Nachricht", "KUU0004",
					"der Betreff", "deutsch");
	...
	private static void juggler() {
		...
		bridge.getDistributor().addTimeLanguageListener(
				new XMPPMessageTimeListenerLanguage() {

					@Override
					public void newMessage(String user, String message,
							String chat, Date time, String language) {
						System.out.println("atoken: " + time.toString() + ": "
								+ user + ": " + message);
					}
				});
		...
		secondBridge.getDistributor().addTimeLanguageListener(
				new XMPPMessageTimeListenerLanguage() {

					@Override
					public void newMessage(String user, String message,
							String chat, Date time, String language) {
						System.out.println("anothertoken: " + time.toString()
								+ ": " + user + ": " + message);
					}
				});
		...
		secondBridge.sendMessageToMultiUserChat(chat, "a other log message",
				null);
